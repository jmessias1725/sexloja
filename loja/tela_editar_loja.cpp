...
void tela_editar_loja::definir_dados_loja(loja *cad_loja){

    QGraphicsScene *cena = new QGraphicsScene;
    informacao_cad_loja = cad_loja;
    lista_telefone = cad_loja->retorna_lista_telefone();
    lista_operadora = cad_loja->retorna_lista_operadora();

    funcoes_extras icone_operadora;

    ui->le_nome->setText(cad_loja->retornar_nome());
    ui->le_razao_social->setText(cad_loja->retornar_razao_social());
    ui->le_cnpj->setText(cad_loja->retornar_cnpj());
    ui->le_cep->setText(cad_loja->retorna_numero_cep());
    ui->le_rua->setText(cad_loja->retorna_nome_rua());
    ui->le_bairro->setText(cad_loja->retorna_bairro());
    ui->le_cidade->setText(cad_loja->retorna_cidade());
    ui->le_uf->setText(cad_loja->retorna_sigla_estado());
    ui->le_numero->setText(QString::number(cad_loja->retorna_numero_residencia()));
    ui->cb_estado->setCurrentIndex(funcao.retorna_id_estado(cad_loja->retorna_sigla_estado().toStdString()));
    ...
    cena->addPixmap(informacao_cad_loja->retorna_QPixmap_imagem());
...
void tela_editar_loja::on_tb_mais_telefone_clicked()
{
    QLabel *lb_telefone;
    QLabel *lb_operadora;
...
void tela_editar_loja::on_tb_menos_telefone_clicked()
{
...
void tela_editar_loja::definir_mascara_telefone(){
    std::string telefone;
...
void tela_editar_loja::adicionar_telefone(){
    bool ja_cadastrado;
    ja_cadastrado = false;
    std::string telefone;
        ...
        if (cb_operadora->currentText()=="------------------------"){
            ...
            for (int i=0; i<int(lista_telefone.size()); i++){
                if (lista_telefone[i]==telefone){
                    ...
                    msg.addButton("OK", QMessageBox::AcceptRole);
                    msg.setFont(QFont ("Calibri", 11,QFont::Normal, false));
                    msg.setText("\nA Loja já possue esse número de telefone em seu pré-cadastro!");
                    ja_cadastrado = true;
                    msg.exec();
                }
            }
...
void tela_editar_loja::cancelar_telefone(){
...
void tela_editar_loja::on_le_cnpj_editingFinished(){
...
void tela_editar_loja::on_le_cep_editingFinished(){
    aux_cep = new cep;
    bool possui_cep;
...
void tela_editar_loja::on_cb_estado_currentIndexChanged(int index)
{
...
void tela_editar_loja::on_btn_cancelar_clicked()
{
    this->close();
}

void tela_editar_loja::on_btn_confirmar_clicked()
{
