...
void tela_pagamento::definir_dados_ganho_avulso(ganho_avulso *dados_gan){
    funcoes_extras funcao;
    tipo = _receita_avulsa;
    verifica_se_eh_avulso = true;
    verifica_se_eh_compra = false;
    dados_ganho = dados_gan;
    ...
    total_pago = 0.0;
    troco = 0.0;
    total_pagar = dados_ganho->retorna_valor();
...
void tela_pagamento::definir_dados_despesa_avulsa(despesa_avulsa * dados_desp){
    funcoes_extras funcao;
    tipo = _despesa_avulsa;
    verifica_se_eh_avulso = true;
    verifica_se_eh_compra = false;
    dados_despesa = dados_desp;
    ...
    total_pago = 0.0;
    troco = 0.0;
    total_pagar = dados_despesa->retorna_valor();
...
{
    if(verifica_se_eh_avulso){
        if(tipo==_despesa_avulsa){
            funcoes_extras funcao;
            conexao_bd conexao;
            QSqlDatabase bd;
            ...
            if (conexao.conetar_bd()){
                ...
                QSqlQuery consultar_id_despesa(bd);

                int id_despesa;

                //Insere os dados no cadastro da compra
                salvar_dados.prepare("INSERT INTO despesa_avulsa(data,descricao,valor,status) VALUES(:data, :descricao, :valor, :status);");
                salvar_dados.bindValue(":data", dados_despesa->retorna_data());
                salvar_dados.bindValue(":descricao",dados_despesa->retorna_descricao());
                salvar_dados.bindValue(":valor", dados_despesa->retorna_valor());
                salvar_dados.bindValue(":status", dados_despesa->retorna_status());
                salvar_dados.exec();

                //realiza a consulta para determinar  o id da compra.
                consultar_id_despesa.exec("SELECT id_despesa_avulsa FROM despesa_avulsa;");
                if(consultar_id_despesa.last()){
                    id_despesa = consultar_id_despesa.value(0).toInt();
                }
                dados_despesa->alterar_id_despesa_avulsa(id_despesa);
                ...
                if((dinheiro_usado->retorna_valor()+dinheiro_usado->retorna_valor_avista()> 0.0)){
                    if(dinheiro_usado->retorna_valor() > 0.0){
                        ...
                        for (int i=0; i<dinheiro_usado->retorna_num_parcelas(); i++){
                            ...
                            salvar_dados_despesa_dinheiro.bindValue(":descricao", "Parcela em dinheiro de número "+QString::number(i+1)+", referente a despesa avulsa de código: "+QString::number(dados_despesa->retorna_id_despesa_avulsa())+".");
                            ...
                            salvar_dados_despesa_dinheiro.bindValue(":origem", _despesa_avulsa);
                            salvar_dados_despesa_dinheiro.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                    ...
                    if(dinheiro_usado->retorna_valor_avista() > 0.0){
                        ...
                        salvar_dados_despesa_dinheiro.prepare("INSERT INTO despesas(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                        salvar_dados_despesa_dinheiro.bindValue(":descricao", "Valor em dinheiro referente ao pagamento da despesa avulsa de código: "+QString::number(dados_despesa->retorna_id_despesa_avulsa())+".");
                        salvar_dados_despesa_dinheiro.bindValue(":valor", valor_pago);
                        salvar_dados_despesa_dinheiro.bindValue(":data", dados_despesa->retorna_data());
                        salvar_dados_despesa_dinheiro.bindValue(":status", dados_despesa->retorna_status());
                        salvar_dados_despesa_dinheiro.bindValue(":origem", _despesa_avulsa);
                        salvar_dados_despesa_dinheiro.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                        salvar_dados_despesa_dinheiro.exec();
                    }
                ...
                if(cartao_usado->retorna_valor() > 0.0){
                    ...
                    salvar_dados_cartao.bindValue(":origem", _despesa_avulsa);
                    salvar_dados_cartao.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                    ...
                    std::vector< QString > data_parcelas = funcao.determina_parcelas(dados_despesa->retorna_data_QDate(),
                    ...
                    if (primeira_parcela!=valor_parcela){
                        ...
                        salvar_dados_despesa_cartao.bindValue(":descricao", "Parcela de número 1, referente a despesa avulsa de código: "+QString::number(dados_despesa->retorna_id_despesa_avulsa())+".");
                        ...
                        salvar_dados_despesa_cartao.bindValue(":origem", _cartao);
                        salvar_dados_despesa_cartao.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                        salvar_dados_despesa_cartao.exec();
                        for (int i=2; i<=numero_pacelas;i++){
                            //Insere na tabela de despesas a despesas da parcela do cartão.
                            salvar_dados_despesa_cartao.prepare("INSERT INTO despesas(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                            salvar_dados_despesa_cartao.bindValue(":descricao", "Parcela de número "+QString::number(i)+", referente  a despesa avulsa de código: "+QString::number(dados_despesa->retorna_id_despesa_avulsa())+".");
                ...
                if(cheque_usado->retorna_valor() > 0.0){
                    ...
                    salvar_dados_cheque.bindValue(":origem", _despesa_avulsa);
                    salvar_dados_cheque.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                    ...
                    salvar_dados_despesa_cheque.bindValue(":descricao", "Cheque utilizado para efetuar pagamento da despesa avulsa de código: "+QString::number(dados_despesa->retorna_id_despesa_avulsa())+".");
                    ...
                    salvar_dados_despesa_cheque.bindValue(":origem", _cheque);
                    salvar_dados_despesa_cheque.bindValue(":id_origem", dados_despesa->retorna_id_despesa_avulsa());
                    salvar_dados_despesa_cheque.exec();
                }

                if((salvar_dados.lastError().number()<=0)&&
                   ...
                   (salvar_dados_avista.lastError().number()<=0)&&
                   (salvar_dados_parcelado.lastError().number()<=0)){
            ...
            if(tipo==_receita_avulsa){
                ...
                if (conexao.conetar_bd()){
                    ...
                    QSqlQuery salvar_dados(bd);
                    QSqlQuery salvar_dados_cartao(bd);
                    QSqlQuery salvar_dados_ganho_cartao(bd);
                    QSqlQuery salvar_dados_cheque(bd);
                    QSqlQuery salvar_dados_ganho_cheque(bd);
                    QSqlQuery salvar_dados_ganho_dinheiro(bd);
                    QSqlQuery salvar_dados_avista(bd);
                    QSqlQuery salvar_dados_parcelado(bd);
                    ...
                    QSqlQuery consultar_id_parcelado(bd);
                    QSqlQuery consultar_id_avista(bd);
                    QSqlQuery consultar_id_ganho(bd);

                    int id_ganho;

                    //Insere os dados no cadastro da compra
                    salvar_dados.prepare("INSERT INTO ganho_avulso(data,descricao,valor,status) VALUES(:data, :descricao, :valor, :status);");
                    salvar_dados.bindValue(":data", dados_ganho->retorna_data());
                    salvar_dados.bindValue(":descricao",dados_ganho->retorna_descricao());
                    salvar_dados.bindValue(":valor", dados_ganho->retorna_valor());
                    salvar_dados.bindValue(":status", dados_ganho->retorna_status());
                    salvar_dados.exec();

                    //realiza a consulta para determinar  o id da compra.
                    consultar_id_ganho.exec("SELECT id_ganho_avulso FROM ganho_avulso;");
                    if(consultar_id_ganho.last()){
                        id_ganho = consultar_id_ganho.value(0).toInt();
                    }
                    dados_ganho->alterar_id_ganho_avulso(id_ganho);
                    ...
                    if((dinheiro_usado->retorna_valor()+dinheiro_usado->retorna_valor_avista()> 0.0)){
                        if(dinheiro_usado->retorna_valor() > 0.0){
                            ...
                            std::vector< parcela * > parcelamento = dinheiro_usado->retorna_parcelamento();
                            for (int i=0; i<dinheiro_usado->retorna_num_parcelas(); i++){
                                salvar_dados_ganho_dinheiro.prepare("INSERT INTO ganhos(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                                salvar_dados_ganho_dinheiro.bindValue(":descricao", "Parcela em dinheiro de número "+QString::number(i+1)+", referente a receita avulsa de código: "+QString::number(dados_ganho->retorna_id_ganho_avulso())+".");
                                salvar_dados_ganho_dinheiro.bindValue(":valor", parcelamento[i]->retorna_valor());
                                salvar_dados_ganho_dinheiro.bindValue(":data", parcelamento[i]->retorna_data());
                                salvar_dados_ganho_dinheiro.bindValue(":status", _aberta);
                                salvar_dados_ganho_dinheiro.bindValue(":origem", _receita_avulsa);
                                salvar_dados_ganho_dinheiro.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                                salvar_dados_ganho_dinheiro.exec();
                            }
                        }
                        ...
                        if(dinheiro_usado->retorna_valor_avista() > 0.0){
                            ...
                            //Insere na tabela de ganhos o ganho da parcela do cartão.
                            salvar_dados_ganho_dinheiro.prepare("INSERT INTO ganhos(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                            salvar_dados_ganho_dinheiro.bindValue(":descricao", "Valor em dinheiro referente ao recebimento da receita avulsa de código: "+QString::number(dados_ganho->retorna_id_ganho_avulso())+".");
                            salvar_dados_ganho_dinheiro.bindValue(":valor", valor_pago);
                            salvar_dados_ganho_dinheiro.bindValue(":data", dados_ganho->retorna_data());
                            salvar_dados_ganho_dinheiro.bindValue(":status", dados_ganho->retorna_status());
                            salvar_dados_ganho_dinheiro.bindValue(":origem", _receita_avulsa);
                            salvar_dados_ganho_dinheiro.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                            salvar_dados_ganho_dinheiro.exec();
                        }
                        ...
                        double valor_pago = dinheiro_usado->retorna_valor()+dinheiro_usado->retorna_valor_avista();
                        //Insere os dados referente ao dinheiro
                        salvar_dados_ganho_dinheiro.prepare("INSERT INTO dinheiro(valor,origem,id_origem,id_pag_avista,id_pag_parcelado) VALUES(:valor, :origem, :id_origem, :id_pag_avista, :id_pag_parcelado);");
                        salvar_dados_ganho_dinheiro.bindValue(":valor", valor_pago);
                        salvar_dados_ganho_dinheiro.bindValue(":origem", _receita_avulsa);
                        salvar_dados_ganho_dinheiro.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                        salvar_dados_ganho_dinheiro.bindValue(":id_pag_avista", id_pag_avista);
                        salvar_dados_ganho_dinheiro.bindValue(":id_pag_parcelado", id_pag_parcelado);
                        salvar_dados_ganho_dinheiro.exec();
                    }
                    ...
                    if(cartao_usado->retorna_valor() > 0.0){
                        ...
                        salvar_dados_cartao.bindValue(":num_parcelas", cartao_usado->retorna_num_parcelas());
                        salvar_dados_cartao.bindValue(":valor", cartao_usado->retorna_valor());
                        salvar_dados_cartao.bindValue(":origem", _receita_avulsa);
                        salvar_dados_cartao.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                        salvar_dados_cartao.exec();
                        ...
                        std::vector< QString > data_parcelas = funcao.determina_parcelas(dados_ganho->retorna_data_QDate(),
                                                                                         cartao_usado->retorna_dia_vencimento(),
                                                                                         cartao_usado->retorna_num_parcelas());

                        if (primeira_parcela!=valor_parcela){
                            //Insere na tabela de despesas a despesas da parcela do cartão.
                            salvar_dados_ganho_cartao.prepare("INSERT INTO ganhos(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                            salvar_dados_ganho_cartao.bindValue(":descricao", "Parcela de número 1, referente a receita avulsa de código: "+QString::number(dados_ganho->retorna_id_ganho_avulso())+".");
                            salvar_dados_ganho_cartao.bindValue(":valor", primeira_parcela);
                            salvar_dados_ganho_cartao.bindValue(":data", data_parcelas[0]);
                            salvar_dados_ganho_cartao.bindValue(":status", _aberta);
                            salvar_dados_ganho_cartao.bindValue(":origem", _cartao);
                            salvar_dados_ganho_cartao.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                            salvar_dados_ganho_cartao.exec();
                            for (int i=2; i<=numero_pacelas;i++){
                                //Insere na tabela de despesas a despesas da parcela do cartão.
                                salvar_dados_ganho_cartao.prepare("INSERT INTO ganhos(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                                salvar_dados_ganho_cartao.bindValue(":descricao", "Parcela de número "+QString::number(i)+", referente a receita avulsa de código: "+QString::number(dados_ganho->retorna_id_ganho_avulso())+".");
                                salvar_dados_ganho_cartao.bindValue(":valor", valor_parcela);
                                salvar_dados_ganho_cartao.bindValue(":data", data_parcelas[i-1]);
                                salvar_dados_ganho_cartao.bindValue(":status", _aberta);
                                salvar_dados_ganho_cartao.bindValue(":origem", _cartao);
                                salvar_dados_ganho_cartao.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                                salvar_dados_ganho_cartao.exec();
                    ...
                    if(cheque_usado->retorna_valor() > 0.0){
                        ...
                        salvar_dados_cheque.bindValue(":numero", cheque_usado->retorna_numero());
                        salvar_dados_cheque.bindValue(":valor", cheque_usado->retorna_valor());
                        salvar_dados_cheque.bindValue(":origem", _receita_avulsa);
                        salvar_dados_cheque.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                        salvar_dados_cheque.bindValue(":data_pagamento", cheque_usado->retorna_data_pagamento());
                        salvar_dados_cheque.bindValue(":nome_banco", cheque_usado->retorna_nome_banco());
                        salvar_dados_cheque.exec();

                        //Insere na tabela de despesas a despesa do cheque.
                        salvar_dados_ganho_cheque.prepare("INSERT INTO ganhos(data,descricao,valor,status,origem,id_origem) VALUES(:data, :descricao, :valor, :status, :origem, :id_origem);");
                        salvar_dados_ganho_cheque.bindValue(":descricao", "Cheque utilizado para efetuar pagamento da receita avulsa de código: "+QString::number(dados_ganho->retorna_id_ganho_avulso())+".");
                        salvar_dados_ganho_cheque.bindValue(":valor", cheque_usado->retorna_valor());
                        if(cheque_usado->retorna_se_insere_caixa_hoje()==true){
                            QDate data_atual;
                            salvar_dados_ganho_cheque.bindValue(":status", _aberta);
                            salvar_dados_ganho_cheque.bindValue(":data", data_atual.currentDate().toString(Qt::SystemLocaleShortDate));
                        }
                        else{
                            salvar_dados_ganho_cheque.bindValue(":data", cheque_usado->retorna_data_pagamento());
                            salvar_dados_ganho_cheque.bindValue(":status", _aberta);
                        }
                        salvar_dados_ganho_cheque.bindValue(":origem", _cheque);
                        salvar_dados_ganho_cheque.bindValue(":id_origem", dados_ganho->retorna_id_ganho_avulso());
                        salvar_dados_ganho_cheque.exec();
                    }

                    if((salvar_dados.lastError().number()<=0)&&
                       (salvar_dados_cartao.lastError().number()<=0)&&
                       (salvar_dados_ganho_cartao.lastError().number()<=0)&&
                       (salvar_dados_cheque.lastError().number()<=0)&&
                       (salvar_dados_ganho_cheque.lastError().number()<=0)&&
                       (salvar_dados_ganho_dinheiro.lastError().number()<=0)&&
                       (salvar_dados_avista.lastError().number()<=0)&&
                       (salvar_dados_parcelado.lastError().number()<=0)){
            ...
            if ((total_pagar-total_pago)>0){
                ...
                if (conexao.conetar_bd()){
                    ...
                    if(verifica_se_eh_estorno == false){
                        ...
                        QSqlQuery remover_lista_compra(bd);
                        ...
                        id_compra = dados_compra->retorna_id_compra();

                        //Insere os dados no cadastro da venda
                        salvar_dados_compra.prepare("UPDATE compra SET data_compra=:data_compra, id_fornecedor=:id_fornecedor, num_cupom_nota=:num_cupom_nota, valor_total=:valor_total, desconto=:desconto, valor_pago=:valor_pago , status=:status WHERE id_compra = '"+QString::number(dados_compra->retorna_id_compra())+"';");
                        salvar_dados_compra.bindValue(":data_compra", dados_compra->retorna_data_compra());
                        salvar_dados_compra.bindValue(":id_fornecedor",dados_compra->retorna_id_fornecedor());
                        salvar_dados_compra.bindValue(":num_cupom_nota",dados_compra->retorna_num_cupom_nota());
                        salvar_dados_compra.bindValue(":valor_total", dados_compra->retorna_valor_total());
                        ...
                        remover_dados_dinheiro.exec("DELETE FROM dinheiro WHERE origem = '"+QString::number(_compra_de_produto)+"' AND id_origem = '"+QString::number(dados_compra->retorna_id_compra())+"';");
                        remover_dados_cartao.exec("DELETE FROM cartao WHERE origem = '"+QString::number(_compra_de_produto)+"' AND id_origem = '"+QString::number(dados_compra->retorna_id_compra())+"';");
                        remover_dados_cheque.exec("DELETE FROM cheque WHERE origem = '"+QString::number(_compra_de_produto)+"' AND id_origem = '"+QString::number(dados_compra->retorna_id_compra())+"';");
                        remover_dados_despesas.exec("DELETE FROM despesas WHERE id_origem = '"+QString::number(dados_compra->retorna_id_compra())+"';");
                    ...
                    if((dinheiro_usado->retorna_valor()+dinheiro_usado->retorna_valor_avista()> 0.0)){
                        if(dinheiro_usado->retorna_valor() > 0.0){
                            ...
                            for (int i=0; i<dinheiro_usado->retorna_num_parcelas(); i++){
                                ...
                                salvar_dados_despesa_dinheiro.bindValue(":data", parcelamento[i]->retorna_data());
                                salvar_dados_despesa_dinheiro.bindValue(":status", _aberta);
                                salvar_dados_despesa_dinheiro.bindValue(":origem", _dinheiro_par);
                        ...
                        if(dinheiro_usado->retorna_valor_avista() > 0.0){
                            ...
                            salvar_dados_despesa_dinheiro.bindValue(":origem", _dinheiro_avi);
                            salvar_dados_despesa_dinheiro.bindValue(":id_origem", dados_compra->retorna_id_compra());
                            salvar_dados_despesa_dinheiro.exec();
                        }
                        ...
                        salvar_dados_despesa_dinheiro.prepare("INSERT INTO dinheiro(valor,origem,id_origem,id_pag_avista,id_pag_parcelado) VALUES(:valor, :origem, :id_origem, :id_pag_avista, :id_pag_parcelado);");
                        salvar_dados_despesa_dinheiro.bindValue(":valor", valor_pago);
                        salvar_dados_despesa_dinheiro.bindValue(":origem", _compra_de_produto);
                        salvar_dados_despesa_dinheiro.bindValue(":id_origem", dados_compra->retorna_id_compra());
                        salvar_dados_despesa_dinheiro.bindValue(":id_pag_avista", id_pag_avista);
                        salvar_dados_despesa_dinheiro.bindValue(":id_pag_parcelado", id_pag_parcelado);
                        salvar_dados_despesa_dinheiro.exec();
                    ...
                    if(cartao_usado->retorna_valor() > 0.0){
                        ...
                        salvar_dados_cartao.bindValue(":num_parcelas", cartao_usado->retorna_num_parcelas());
                        salvar_dados_cartao.bindValue(":valor", cartao_usado->retorna_valor());
                        salvar_dados_cartao.bindValue(":origem", _compra_de_produto);
                    ...
                    if(cheque_usado->retorna_valor() > 0.0){
                        ...
                        salvar_dados_cheque.bindValue(":numero", cheque_usado->retorna_numero());
                        salvar_dados_cheque.bindValue(":valor", cheque_usado->retorna_valor());
                        salvar_dados_cheque.bindValue(":origem", _compra_de_produto);
                    ...
                    for(int i=0; i<int(lt_compra.size());i++){
                        ...
                        if(encontrou_valor_compra == false){
                            ...
                            salvar_dados_his_balanco_estoque.clear();

                            //realiza a consulta para determinar o id do balanco.
                            consultar_id_balanco.exec("SELECT id_balanco FROM his_balanco_estoque WHERE id_produto = '"+QString::number(lt_compra[i]->retorna_id_produto())+"';");
                    ...
                    if((salvar_dados_compra.lastError().number()<=0)&&
                       (alterar_dados_his_balanco_estoque.lastError().number()<=0)&&
                       (salvar_dados_his_balanco_estoque.lastError().number()<=0)&&
                       (salvar_dados_lista_compra.lastError().number()<=0)&&
                       (atualiza_valor_venda_produto.lastError().number()<=0)&&
                       (salvar_dados_cheque.lastError().number()<=0)&&
                       (salvar_dados_cartao.lastError().number()<=0)&&
                       (salvar_dados_despesa_cheque.lastError().number()<=0)&&
            ...
            if ((total_pagar-total_pago)>0){
                ...
                if (conexao.conetar_bd()){
                    ...
                    if(verifica_se_eh_estorno  == false){
                        ...
                        id_venda = dados_venda->retorna_id_venda();
                        ...
                        remover_dados_dinheiro.exec("DELETE FROM dinheiro WHERE origem = '"+QString::number(_venda_de_produto)+"' AND id_origem = '"+QString::number(dados_venda->retorna_id_venda())+"';");
                        remover_dados_cartao.exec("DELETE FROM cartao WHERE origem = '"+QString::number(_venda_de_produto)+"' AND id_origem = '"+QString::number(dados_venda->retorna_id_venda())+"';");
                        remover_dados_cheque.exec("DELETE FROM cheque WHERE origem = '"+QString::number(_venda_de_produto)+"' AND id_origem = '"+QString::number(dados_venda->retorna_id_venda())+"';");
                        ...
                        remover_pagamento_parcelado.exec("DELETE FROM pagamento_parcelado WHERE id_pag_parcelado = '"+QString::number(id_parcelado)+"';");
                        remover_justificativa_cancelamento.exec("DELETE FROM jus_cancelamento_nota WHERE origem = '"+QString::number(_nota_de_venda)+"' AND id_origem = '"+QString::number(dados_venda->retorna_id_venda())+"';");
                    ...
                    if((dinheiro_usado->retorna_valor()+dinheiro_usado->retorna_valor_avista()> 0.0)){
                        if(dinheiro_usado->retorna_valor() > 0.0){
                            ...
                            for (int i=0; i<dinheiro_usado->retorna_num_parcelas(); i++){
                                ...
                                salvar_dados_dinheiro.bindValue(":origem", _dinheiro_par);
                        ...
                        if(dinheiro_usado->retorna_valor_avista() > 0.0){
                            ...
                            salvar_dados_dinheiro.bindValue(":origem",_dinheiro_avi);
                            salvar_dados_dinheiro.bindValue(":id_origem", dados_venda->retorna_id_venda());
                            salvar_dados_dinheiro.exec();
                        }
                    ...
                    if(cheque_usado->retorna_valor() > 0.0){
                        ...
                        salvar_dados_cheque.bindValue(":numero", cheque_usado->retorna_numero());
                        salvar_dados_cheque.bindValue(":valor", cheque_usado->retorna_valor());
                        salvar_dados_cheque.bindValue(":origem", _venda_de_produto);
