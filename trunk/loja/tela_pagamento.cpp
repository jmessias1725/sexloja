    ...
    if (conexao.conetar_bd("localhost",3306,"bd_loja","root","tiger270807","tela_pagamento::on_btn_confirmar_clicke")){
        ...
        for(int i=0; i<int(lt_compra.size());i++){
            ...
            while((consultar_valor_compra.next())&&(encontrou_valor_compra==false)){
                ...
                if(valor_compra==lt_compra[i]->retorna_valor_compra()){
                    encontrou_valor_compra = true;

                    campos = "valor_compra=:valor_compra, id_produto=:id_produto, total_comprado=:total_comprado, total_disponivel=:total_disponivel";
                    ...
                    alterar_dados_his_balanco_estoque.prepare("UPDATE his_balanco_estoque SET "+campos+" WHERE id_produto = '"+QString::number(lt_compra[i]->retorna_id_produto())+"';");
                    alterar_dados_his_balanco_estoque.bindValue(":valor_compra", lt_compra[i]->retorna_valor_compra());
                    alterar_dados_his_balanco_estoque.bindValue(":id_produto",lt_compra[i]->retorna_id_produto());
            ...
            if(encontrou_valor_compra == false){
                ...
                consultar_id_balanco.exec("SELECT id_compra FROM compra");
