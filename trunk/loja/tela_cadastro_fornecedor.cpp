...
#include "ui_tela_cadastro_fornecedor.h"

tela_cadastro_fornecedor::tela_cadastro_fornecedor(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::tela_cadastro_fornecedor)
{
    ui->setupUi(this);
}

tela_cadastro_fornecedor::~tela_cadastro_fornecedor()
{
    delete ui;
}
...
void tela_cadastro_fornecedor::on_tb_mais_email_clicked()
{
...
void tela_cadastro_fornecedor::adicionar_email(){
...
void tela_cadastro_fornecedor::cancelar_email(){
    if (le_email->text()=="")
    {
        tela_cadastro_email->close();
    }
...
void tela_cadastro_fornecedor::on_tb_mais_telefone_clicked()
{
    QLabel *lb_telefone;
    QLabel *lb_operadora;
...
void tela_cadastro_fornecedor::on_tb_menos_telefone_clicked()
{
...
void tela_cadastro_fornecedor::definir_mascara_telefone(){
    std::string telefone;
...
void tela_cadastro_fornecedor::adicionar_telefone(){
    bool ja_cadastrado;
    ja_cadastrado = false;
    std::string telefone;
        ...
        if (cb_operadora->currentText()=="------------------------"){
            ...
            for (int i=0; i<int(lista_telefone.size()); i++){
                if (lista_telefone[i]==telefone){
                    ...
                    msg.addButton("OK", QMessageBox::AcceptRole);
                    msg.setFont(QFont ("Calibri", 11,QFont::Normal, false));
                    msg.setText("\nO Forncedor já possue esse número de telefone em seu pré-cadastro!");
                    ja_cadastrado = true;
                    msg.exec();
                }
            }
...
void tela_cadastro_fornecedor::cancelar_telefone(){
...
void tela_cadastro_fornecedor::on_cb_estado_currentIndexChanged(int index)
{
...
void tela_cadastro_fornecedor::on_btn_cancelar_clicked()
{
    this->close();
}

void tela_cadastro_fornecedor::on_btn_confirmar_clicked()
{
    if((ui->le_nome->text().toStdString()!="")&&(lista_telefone.size()>0)){
        ...
        cad_fornecedor->definir_icone_janela(logomarca);
...
void tela_cadastro_fornecedor::closeEvent( QCloseEvent * event ){
